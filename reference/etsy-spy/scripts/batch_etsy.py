#!/usr/bin/env python3
"""
Batch Etsy search and auto-add qualified listings.
"""
...
def run_search(keyword, limit=20):
...
def add_listing(listing, keyword):
    cmd = [sys.executable, "etsy_backlog.py", "add",
           ...
           "--design-angle", "Batch auto-qualified",
           "--workflow-status", "spied"]
    try:
        subprocess.run(cmd, timeout=30, cwd=os.path.dirname(__file__))
        return True
    except:
        return False
    ...
    for kw in KEYWORDS:
        ...
        for cat in ['hot', 'watch']:
            for lst in data.get(cat, []):
                if qualify(lst.get('sold_24h',0), lst.get('views_24h',0),
